from geopy.distance import geodesic  # vincenty公式  精度很高能达到0.5毫米，但是很慢
import pandas as pd
import math
import numpy as np


driver = open(r'.\司机订单大于20的集合\11-09\大于20的司机id11-09.txt')
row = 1
for driver_id in driver:
    driver_id = driver_id.rstrip('\n')
    with open(r'.\司机订单大于20的集合\11-09\司机' + driver_id + '.csv', encoding='utf-8') as f:
        ...
        for i in range(len(y) - 1):
            ...
            if data_after[1] != data_before[1]:
                ...
                tempspeed = geodesic(before_gps[::-1], after_gps[::-1]).meters / time[i]
        ...
        columns_name = ['driver_id', 'num_order', 'average_order_num',
        ...
        df.loc[0] = [driver_id, new_order_num, len(y) / new_order_num,
                     ...
                     v_more_167_num / v_exist_num, a_v_more_pos02_num / a_v_exist_num,
                     a_v_more_neg02_num / a_v_exist_num,
...
# 把标签存入字典，并存入文件
label = {}
with open('标签.csv', mode='r', encoding='utf-8') as f:
    y = pd.read_csv(f, header=None)
    print(y)
    for i in range(len(y)):
        label[y.values[i][0]] = y.values[i][-1]
ff = open("label.txt", 'w')
ff.write(str(label))
ff.close()
