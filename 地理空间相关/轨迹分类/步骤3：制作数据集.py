...
with open("label.txt", 'r') as ff:  # label2.txt是没有加权值的标签
    label = eval(ff.read())
print(len(label))
    ...
    with open(r'.\司机订单大于20的集合\11-09\司机' + driver_id + '.csv', encoding='utf-8') as f:
        ...
        for i in range(len(y) - 1):
            ...
            if data_after[1] != data_before[1]:
                ...
                tempSpeed.append(geodesic(before_gps[::-1], after_gps[::-1]).meters / timelist[i])
...
np.save(r'.\司机订单大于20的集合\11-09\label_arrays2.npy', label_array)
np.save(r'.\司机订单大于20的集合\11-09\all_arrays2.npy', all_arrays)
