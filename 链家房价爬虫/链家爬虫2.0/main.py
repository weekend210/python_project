...
# 整合房价字典
def merge_price_dict(url, pagekey, PageStart, PageEnd, strkey, re_data_dict):
    initial = {}
    ...
    re_data_dict[strkey] = initial    # 加入返回值的列表中
    print("线程：", os.getpid(), '数据爬取完成')
    return initial


# 写入json文件
def writer_in_json(data, json_name):
    count = 1
...
# 把房价数据和POI数据匹配，生成新字典
def match_price_POI(hp_infor, City, ak, re_data_dict):
    count = 1
...
if __name__ == '__main__':
    ...
    re_data_dict = manager.dict()  # 进程列表，用于获取返回值

    # 每个线程爬取房价的页数已经做到数据拆分
    process_1 = multiprocessing.Process(target=merge_price_dict,
                                        args=(url, 'cro21/', 1, 7, '房价升序1-50', re_data_dict))   # 页数设置1-50
    '''
    process_2 = multiprocessing.Process(target=merge_price_dict,
                                        args=(url, 'cro21/', 51, 51, '房价升序51-100', re_data_dict))   # 页数设置51-100
    process_3 = multiprocessing.Process(target=merge_price_dict,
                                        args=(url, 'cro22/', 1, 1, '房价降序1-50', re_data_dict))   # 页数设置1-50
    process_4 = multiprocessing.Process(target=merge_price_dict,
                                        args=(url, 'cro22/', 51, 51, '房价降序51-100', re_data_dict))   # 页数设置51-100
    process_5 = multiprocessing.Process(target=merge_price_dict,
                                        args=(url, 'cro22p3/', 1, 1, '房价1w-1.5w段降序1-55', re_data_dict)) # 页数设置1-55
    process_6 = multiprocessing.Process(target=merge_price_dict,
                                        args=(url, 'cro22p3/', 56, 56, '房价1w-1.5w段降序56-80', re_data_dict))  # 页数设置56-80
    process_7 = multiprocessing.Process(target=merge_price_dict,
                                        args=(url, 'cro21p4/', 1, 1, '房价1.5w-2w段正序1-30', re_data_dict)) # 页数设置1-30
    '''
    process_1.start()
    '''
    process_2.start()
    ...
    # 合并数据线程
    for_write_in_json = {}
    # 将爬取的房价数据从Manager中提取出来并合并
    for key in re_data_dict:
        for_write_in_json.update(re_data_dict[key])

    process_x = multiprocessing.Process(target=writer_in_json,
                                        args=(for_write_in_json, '武汉房价.json'))
    ...
    print(f'-----POI爬取结束，开始写入csv文件-----')
    print(f'----------------------------------')

    # 写入文件
    # 合并数据
    re_data = {}  # 存储所有爬取的数据，用于写入csv
    for i in re_data_list:
        re_data.update(i)
